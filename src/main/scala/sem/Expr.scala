...
//    case BlockExpr(_, expr, _, _, _) => expr.returnType
...
//    case BlockExpr(exprs, expr, _, _, _) =>
//      exprs.foreach(_.gatherFuns(funs))
//      expr.gatherFuns(funs)
...
//    case BlockExpr(exprs, expr, vars, _, _) => vars ::: expr.gatherLocals ::: exprs.flatMap(_.gatherLocals)
...
//    case BlockExpr(exprs, expr, _, _, _) => expr.runtime && exprs.forall(_.runtime)
...
//    case BlockExpr(exprs, expr, _, _, _) => expr.compiletime && exprs.forall(_.compiletime)
...
//    case RefGlobalVarExpr(globalVar, _, _) => None
//    case LocalVarExpr(localVar, _, _) => localVar.constVal
//    case RefLocalVarExpr(localVar, _, _) => None
//    case ValExpr(expr, _, _) => None
...
//    case BlockExpr(exprs, expr, _, _, _) => ctx.scope(if exprs.map(_.constEval(ctx)).forall(_.nonEmpty) then expr.constEval(ctx) else None)
//    case UnitExpr(_, _) => Some(ConstUnit)
...
//    case funDatatype: FunDatatype if args.map(_.returnType).zip(funDatatype.params).forall(_ isSubtypeOf _) => Some((variable, args))
//    case _ => None
//  }
//
...
//  case syn.BlockExpr(exprs, expr, range) =>
//    val ((analyzedExprs, analyzedLastExpr), vars) = ctx.scope((exprs.map(analyzeExpr(ctx)), analyzeExpr(ctx)(expr)))
